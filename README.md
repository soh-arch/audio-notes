...
原稿は `# 見出し` で始めてください。この見出しがページのタイトル（ブラウザのタブと本文冒頭の見出し）になります。タイトルの入力箇所をここ1箇所に限ることで、Markdownで読んだときとページで見たときでタイトルが食い違うことを防いでいます。見出しが無い原稿はエラーになります。
...
唯一確実に動くのが、**本文が最初からHTMLソースに書き込まれた、装飾の少ない静的ページ**をGitHub Pagesで配信する形でした。この形式を実機（iPhone版Edge）で検証した上で、`build_note.py` と `note-template.html` を実装しています。テンプレートを変更するときは、本文が最初からHTMLソースに入っているというこの性質だけは崩さないでください。本文をJavaScriptで後から差し込む形に戻すと、読み上げが動かなくなります。
